...
class ToDoWidget: Widget, UITableViewDelegate, UITableViewDataSource  {
   ...
   override init(frame: CGRect) {
    ...
    init(size: Int, PHAccessed: Array<PlaceHolder>, items: [String]) {
        ...
        let centerX = self.calculateCoords(PHA: PHAccessed).x
        let centerY = self.calculateCoords(PHA: PHAccessed).y
        
        self.center = CGPoint(x: centerX, y: centerY)
        self.ogCenter = self.center
        
        self.ogPosition = CGPoint(x: PHAccessed[0].posX, y: PHAccessed[0].posY)
