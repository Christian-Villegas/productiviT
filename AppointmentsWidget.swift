...
class AppointmentsWidget: Widget, UITableViewDelegate, UITableViewDataSource {
    ...
    @objc override func addButton(_ sender: UIButton!) {
        ...
        let action = UIAlertAction(title: "Continue", style: .default)
        {
            ...
            //Code for adding appt///
            let content = UNMutableNotificationContent()
            content.title = newAppt.title.text!
            content.body = "Did you accomplish your goal?"
            content.sound = UNNotificationSound.default
            let current = Date()
            let temp = current.distance(to: newAppt.date)
            let trigger = UNTimeIntervalNotificationTrigger(timeInterval: temp, repeats: false)
